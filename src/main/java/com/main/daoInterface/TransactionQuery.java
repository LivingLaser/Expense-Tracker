...
public interface TransactionQuery {
	...
	String INCOMES = "select uid, type, name, amount, icon, DATE_FORMAT(date, '%d-%m-%Y') as date, curr_date, time from transaction where uid=? and type='income' order by STR_TO_DATE(date, '%Y-%m-%d') desc";
	String EXPENSES = "select uid, type, name, amount, icon, DATE_FORMAT(date, '%d-%m-%Y') as date, curr_date, time from transaction where uid=? and type='expense' order by STR_TO_DATE(date, '%Y-%m-%d') desc";
	...
	String UPDATE_RECORD = "update transaction set name=?, amount=?, icon=?, date=? where uid=? and type=? and curr_date=? and time=?";
	String DELETE_RECORD = "delete from transaction where uid=? and type=? and curr_date=? and time=?";
